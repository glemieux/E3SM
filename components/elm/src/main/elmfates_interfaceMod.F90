...
contains
         ...
         if (col_pp%is_fates(c)) then
            ...
            case (prt_cnp_flex_allom_hyp )
               ...
               col_pf%decomp_ppools_sourcesink(c,1:nlevdecomp,i_met_lit) =  &
                    col_pf%decomp_ppools_sourcesink(c,1:nlevdecomp,i_met_lit) * dtime
               col_pf%decomp_ppools_sourcesink(c,1:nlevdecomp,i_cel_lit) =  &
                    col_pf%decomp_ppools_sourcesink(c,1:nlevdecomp,i_cel_lit) * dtime
               col_pf%decomp_ppools_sourcesink(c,1:nlevdecomp,i_lig_lit) =  &
                    col_pf%decomp_ppools_sourcesink(c,1:nlevdecomp,i_lig_lit) * dtime

               col_nf%decomp_npools_sourcesink(c,1:nlevdecomp,i_met_lit) =  &
                    col_nf%decomp_npools_sourcesink(c,1:nlevdecomp,i_met_lit) * dtime
 ...
 subroutine wrap_update_hifrq_hist(this, bounds_clump )
    ...
    associate(&
      ...
      ! Update model timestep (hifreq) interface variables for fates history
      call this%fates(nc)%UpdateInterfaceVariablesTimeStep(update_history=.true.)
   ...
   do r = 1, this%fates(nc)%npatches
      ...
      if (use_fates_planthydro) then
         ...
         call this%fates(nc)%registry(r)%Register(key=hlm_fates_soil_water_vol_min, &
                                                  data=soilstate_inst%watmin_col(c,:), &
                                                  hlm_flag=.true., &
                                                  subgrid_type=registry_var_intid_column)
      end if
